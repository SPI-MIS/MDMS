...
router.get('/material', async (req, res) => {
  ...
  try {
    ...
    let where = 'WHERE (MC001 LIKE @q)';
    if (state) {
      request.input('state', sql.NVarChar, state);
      where += ' AND IssueState = @state';
    }
    const rs = await request.query(`
      SELECT MC001, MC002, IssueState, Creator, CreateDate
      FROM dbo.SMSMC
      ${where}
      ORDER BY MC001
    `);
...
router.post('/material', async (req, res) => {
  ...
  const { MC001, MC002 = '', Creator = '' } = req.body;
  
  // 基本驗證
  if (!MC001 || typeof MC001 !== 'string' || !MC001.trim()) {
    console.log('MC001 驗證失敗:', MC001);
    return res.status(400).json({ error: 'MC001 is required and must be a non-empty string' });
  }
  ...
  try {
    ...
    const request = pool.request();
    request.input('MC001', sql.NVarChar, MC001.trim());
    request.input('MC002', sql.NVarChar, MC002);
    request.input('Creator', sql.NVarChar, Creator || 'SYSTEM');
    
    const result = await request.query(`
      INSERT INTO dbo.SMSMC(MC001, MC002, IssueState, Creator, CreateDate)
      VALUES(@MC001, @MC002, 'N', @Creator, GETDATE())
    `);
...
router.put('/material/:id', async (req, res) => {
  ...
  try {
    ...
    const rs = await pool.request()
      ...
      .query(`SELECT IssueState FROM dbo.SMSMC WHERE MC001=@id`);
...
router.delete('/material/:id', async (req, res) => {
  try {
    ...
    await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`DELETE FROM dbo.SMSMC WHERE MC001=@id`);
...
/** 複製：以既有鍵 + 後綴產生新鍵 */
router.post('/material/:id/copy', async (req, res) => {
  const { newId } = req.body; // 前端提供新 MC001
  try {
    const pool = await getPool();
    const src = await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`SELECT * FROM dbo.SMSMC WHERE MC001=@id`);
    if (src.recordset.length === 0) return res.status(404).json({ error: 'source not found' });

    const r = src.recordset[0];
    await pool.request()
      .input('MC001', sql.NVarChar, newId)
      .input('MC002', sql.NVarChar, r.MC002)
...
router.post('/material/:id/approve', async (req, res) => {
  try {
    const pool = await getPool();
    ...
    await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`UPDATE dbo.SMSMC SET IssueState='Y' WHERE MC001=@id`);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/material/:id/unapprove', async (req, res) => {
  try {
    const pool = await getPool();
    const rs = await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`SELECT IssueState FROM dbo.SMSMC WHERE MC001=@id`);
    ...
    await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`UPDATE dbo.SMSMC SET IssueState='N' WHERE MC001=@id`);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/material/:id/void', async (req, res) => {
  try {
    const pool = await getPool();
    const rs = await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`SELECT IssueState FROM dbo.SMSMC WHERE MC001=@id`);
    ...
    await pool.request().input('id', sql.NVarChar, req.params.id)
      .query(`UPDATE dbo.SMSMC SET IssueState='V' WHERE MC001=@id`);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
